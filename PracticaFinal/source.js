...
var margin = {top: 70, right: 65, bottom: 40, left: 53},
...
d3.csv("mediaHombres.csv").then(function(data) {
	...
	d3.csv("mediaMujeres.csv").then(function(data2){
		...
		d3.csv("mediaDos.csv").then(function(data3){
...
var button3 = document.getElementById("data3")
//Variables muy importante, una varia dependiendo de los botones que pulsemos para saber que graficas hay que visualizar y "barra" indica, en el caso
//de que estamos en la visualizacion de conjunto, si estamos sobre la barra de hombres o la de mujeres cuando movemos el raton
var sexo
var barra
...
var title = svg.append("g")
    ...
    .text("COLESTEROL DE PERSONAS CON UNA ENFERMEDAD CARDIOVASCULAR");
...
//Si pulsamos el boton de conjunto borramos el svg actual, cremaos uno nuevo y llamamos a la funcion que pinta todo de nuevo con los datos del tercer csv
...
function dibujaX(){
	if(sexo == 'M'){
		...
		x.domain(data3.map(function(d) { return d.Edad; }));
	}
		svg.append("g")
		.transition()
		.duration(1000)
...
function dibujaY(){
	if(sexo == 'M'){
		...
		y.domain([0, d3.max(data3, function(d) { return (0, 600); })]);
	}
     svg.append("g")
...
//o a mouseOn2_0 y mouseOn3_0 en caso de estar en la visualizacion de conjunto
function dibujaBarras(){
	...
	svg.selectAll(".bar")
		...
		.attr("y", 0)
		.attr("height", function(d) { return h - y(d.MediaHombres); })
		.attr("stroke","black")
		.attr("fill", colorbarra)
		
		.attr("colesterol", function(d){ return Number.parseFloat(d.MediaHombres).toFixed(1)})
		.on("mouseover", mouseOn2_0)
		.on("mouseout", mouseOut);

		d3.selectAll(".bar")
		.transition()
		.duration(1000)
		.attr("y", function(d) { return y(d.MediaHombres); })

		svg.selectAll(".bar2")
		.data(datos)
		.enter().append("rect")
		.attr("class", "bar2")
		.attr("x", function(d) { return x(d.Edad)+20; })
		.attr("width", x.bandwidth()/2)
		.attr("y", 0)
		.attr("height", function(d) { return h - y(d.MediaMujeres); })
		.attr("stroke","black")
		.attr("fill", colorbarra2)
		.on("mouseover", mouseOn3_0)
		.on("mouseout", mouseOut);

		d3.selectAll(".bar2")
		.transition()
		.duration(1000)
		.attr("y", function(d) { return y(d.MediaMujeres); })
...
//Funcion que se ejecuta si estamos sobre las columnas de hombres en la visualizacion de conjunto y reproduce las animaciones y etiquetas
function mouseOn2_0(event) {
	barra = 1
	d3.select(this)
	.attr("fill", "red")
	.transition()
	.duration(300)
	.attr("stroke","black")
	.attr("y", function(d) { return y(d.MediaHombres)-2.5; })
	.attr("width", x.bandwidth()/2+2.5)
	.attr("height", function(d) { return h - y(d.MediaHombres)+2.5; })
	.attr("x", function(d) { return x(d.Edad)-1.25; })
...
//Funcion que se ejecuta si estamos sobre las columnas de mujeres en la cisualizacion de conjunto y reproduce las animaciones y etiquetas
function mouseOn3_0(event) {
	barra = 2
	d3.select(this)
	.attr("colesterol2", function(d){ return Number.parseFloat(d.MediaMujeres).toFixed(1)})
		.attr("fill", "red")
		.transition()
		.duration(300)
		.attr("x", function(d) { return x(d.Edad)+20-1.25; })
		.attr("stroke","black")
		.attr("y", function(d) { return y(d.MediaMujeres)-2.5; })
		.attr("width", x.bandwidth()/2+2.5)
		.attr("height", function(d) { return h - y(d.MediaMujeres)+2.5; })
...
//generamos el texto de dentro del rectangulo cogiendo el colesterol del atributo "colesterol" añadido anteriormente y le añadimos la misma animacion que al rectangulo
	...
	svg.append('text')
		...
		.attr("font-size",12)
		.attr("opacity", .100)
		.text("Colesterol medio: " + d3.select(this).attr("colesterol2"))
	d3.select(".texto_valor")
	.transition()
	.duration(500)
	.attr("opacity", .8)
...
function mouseOut(event) {
	if(sexo=='M' ||sexo=='F'){
		...
		//si esatmos en visualizacion de conjunto y sobre barras de hombres
		if(barra == 1){
		d3.select(this)
			.transition()
			.duration(300)
			...
			//si esatmos en visualizacion de conjunto y sobre barras de mujeres
		}else{
			d3.select(this)
			.transition()
			.duration(300)
